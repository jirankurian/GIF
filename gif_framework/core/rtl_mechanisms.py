...
class STDP_Rule(PlasticityRuleInterface):
    ...
    def apply(self, **kwargs) -> torch.Tensor:
        ...
        if pre_spikes.dim() != 3:
            raise ValueError(f"pre_spikes must be 3D tensor [time, batch, neurons], got shape {pre_spikes.shape}")
        if post_spikes.dim() != 3:
            raise ValueError(f"post_spikes must be 3D tensor [time, batch, neurons], got shape {post_spikes.shape}")
        ...
        for t in range(time_steps):
            ...
            ltp_update = torch.outer(
                ...
                current_post.mean(dim=0)     # Average across batch
            ...
            ltd_update = torch.outer(
                current_pre.mean(dim=0),     # Average across batch
            ...
            self.pre_trace = self.pre_trace * self.decay_ltp + current_pre
            self.post_trace = self.post_trace * self.decay_ltd + current_post
...
class ThreeFactor_Hebbian_Rule(PlasticityRuleInterface):
    ...
    def __init__(self, learning_rate: float):
        """
        Initialize the Three-Factor Hebbian learning rule.

        Args:
            learning_rate (float): Base learning rate (η) that scales the magnitude
                                 of weight updates. This determines how quickly
                                 synapses adapt to correlated activity when the
                                 modulatory signal is present.
                                 Typical range: 0.001 - 0.1
    ...
    def apply(self, **kwargs) -> torch.Tensor:
        ...
        """
        # Extract required parameters
        if 'pre_activity' not in kwargs:
            raise ValueError("Three-Factor Hebbian rule requires 'pre_activity' parameter")
        if 'post_activity' not in kwargs:
            raise ValueError("Three-Factor Hebbian rule requires 'post_activity' parameter")
        ...
        # Apply three-factor rule: scale by learning rate and modulatory signal
        weight_updates = self.learning_rate * correlation_matrix * modulatory_signal
